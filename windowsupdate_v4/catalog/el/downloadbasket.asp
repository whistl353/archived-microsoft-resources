	...
	<script language="JScript">
		...
		aRows = eSortTable.rows
			...
			for (i=0;i<buttonarray.length;i++){
				if(buttonarray[i].innerHTML == "Προσθήκη")
					buttonarray[i].style.width = addlength
				if(buttonarray[i].innerHTML == "Κατάργηση")
...
<table cellspacing="0" cellpadding="0" border="0">
	<tbody>
	<tr><td height="1"><span class="8pt sys-font-body sys-color-body" id="addspan" name="addspan" style="visibility:hidden;height=1;overflow;hidden">Προσθήκη</span><span class="8pt sys-font-body sys-color-body" name="removespan" id="removespan" style="visibility:hidden;height=1;overflow;hidden">Κατάργηση</span></td></tr>
		<tr>
			<td valign="top" height="40em">
				<div>
					<div valign="middle" style="background-color:#Ffffcc;padding-top:3;padding-bottom:5;padding-left:10">
						...
						<span class="title sys-font-heading3 sys-rhp-color-title">Καλάθι αρχείων για λήψη</span></nobr>
					</div>
				</div></td>
		</tr>
		<tr>
			<td valign="top">
				<div name="reviewandinstall"id="reviewandinstall">
					<div   class="8pt sys-font-body sys-color-body"><b>Ανασκόπηση και λήψη επιλεγμένων ενημερωμένων εκδόσεων</b></div>
					<div  class="8pt sys-font-body sys-color-body"><!--<img src="/catalog/images/ts.gif" height="1" width="56">-->Καθορίστε τη θέση λήψης που προτιμάτε και στη συνέχεια κάντε κλικ στο κουμπί <b>’μεση λήψη</b>. Για να καταργήσετε ένα στοιχείο από το καλάθι σας, κάντε κλικ στο κουμπί <b>Κατάργηση</b>.</div>
				</div>
				<div name="noreviewandinstall"id="noreviewandinstall" style="display:none">
					<div   class="8pt sys-font-body sys-color-body"><b><b>Δεν υπάρχουν ενημερωμένες εκδόσεις στο καλάθι σας</b></b></div>
					<div  class="8pt sys-font-body sys-color-body"><!--<img src="/catalog/images/ts.gif" height="1" width="56">-->Για να επιλέξετε ενημερωμένες εκδόσεις των οποίων θα κάνετε λήψη, κάντε κλικ στην επιλογή <b>Καλώς ορίσατε στον κατάλογο του Windows Update</b> ή σε μια κατηγορία κάτω από αυτήν και υποβάλετε τα κριτήρια αναζήτησης.</div>
				</div>
			</td>
		</tr>
		<tr>
			...
			<form name="txtform" id="textform" action="downloadbasket.asp" onsubmit="return downloadnow();">
				<div>
					<div class="8pt sys-font-body sys-color-body">&nbsp;<br><nobr>Πληκτρολογήστε ή μεταβείτε στη θέση λήψης της επιλογής σας:</nobr></div>
					<div class="10pt sys-font-heading4">
						...
						<button onresize="document.recalc('true')" style="width:expression(browsespan.offsetWidth + 10)" onClick="javascript:goBrowse()" id="browseb" name="browseb">
							<span id="browsespan" name="browsespan">Αναζήτηση...</span></button></nobr>
					...
					<table cellpadding="0" cellspacing="0" border="0">
						<tr>
							<td class="8pt sys-font-body sys-color-body">
								...
								<button id="dnow" name="dnow" type="submit" style="width:expression(downspan.offsetWidth + 10)" disabled><span name="downspan" id="downspan">’μεση λήψη</span></button>
								<br>
							</td>
						</tr>
					</table>
		...
		<tr>
			<td class="resultsBasketBar sys-table-header-bgcolor1 sys-table-color-border sys-toppane-header-color" valign="top">
				<span id="eBasketCalculating">Σύνολο (συμπεριλαμβανομένων των προαπαιτούμενων):</span> <span id="eBasketStats"></span>
			</td>
		</tr>
		<tr>
			<td height="100%">
				<div id="eUpdatesContainer" class="eUpdatesContainer sys-table-color-border sys-table-cell-bgcolor2">
					...
					<div id="noupdates" class="8pt sys-font-body sys-color-body" style="display:none">
						...
						<p>
						<center>
						<b>Δεν υπάρχουν ενημερωμένες εκδόσεις στο καλάθι σας.</b>
						</center>
					</div>
...
//fnSortTable(2,false)
	...
	var Minute="λεπτό";
	var Minutes="λεπτά";
	var DownloadSize="Μέγεθος λήψης:"
	var MB="MB"
	var KB="KB"
	var basketarray=parent.frames["eTOC"].sBasketString.split(",");
...
//alert(controlxml)
		...
		if(bstrStartFolder == "" || false == bstrStartFolder) {
			var badpath="Δεν είναι δυνατή η λήψη στον καθορισμένο φάκελο από το Windows Update. Είτε ο φάκελος δεν υπάρχει είτε δεν έχετε δικαιώματα προσθήκης στοιχείων σε αυτόν. Βεβαιωθείτε ότι έχετε καθορίσει τη σωστή διαδρομή στο φάκελο ή καθορίστε άλλη θέση.";
			alert(badpath);
			return false;
		}
	...
	function goBrowse() {
		...
		if(pbstrFolder.length >=50){
			...
			sAlert = 'Η διαδρομή λήψης πρέπει να περιέχει λιγότερους από 50 χαρακτήρες. Η διαδρομή που καθορίσατε είναι η εξής: \n\r' + pbstrFolder + "\n\r\n\r" + 'Πληκτρολογήστε ή μεταβείτε σε μια διαδρομή λήψης που έχει μήκος μικρότερο των 50 χαρακτήρων.'
			alert(sAlert)
		}
