	...
	<script language="JScript">
		...
		aRows = eSortTable.rows
			...
			for (i=0;i<buttonarray.length;i++){
				if(buttonarray[i].innerHTML == "הוסף")
					buttonarray[i].style.width = addlength
				if(buttonarray[i].innerHTML == "הסר")
...
<table cellspacing="0" cellpadding="0" border="0">
	<tbody>
	<tr><td height="1"><span class="8pt sys-font-body sys-color-body" id="addspan" name="addspan" style="visibility:hidden;height=1;overflow;hidden">הוסף</span><span class="8pt sys-font-body sys-color-body" name="removespan" id="removespan" style="visibility:hidden;height=1;overflow;hidden">הסר</span></td></tr>
		<tr>
			<td valign="top" height="40em">
				<div>
					<div valign="middle" style="background-color:#Ffffcc;padding-top:3;padding-bottom:5;padding-left:10">
						...
						<span class="title sys-font-heading3 sys-rhp-color-title">סל הורדות</span></nobr>
					</div>
				</div></td>
		</tr>
		<tr>
			<td valign="top">
				<div name="reviewandinstall"id="reviewandinstall">
					<div   class="8pt sys-font-body sys-color-body"><b>עבור על העדכונים שבחרת והורד אותם</b></div>
					<div  class="8pt sys-font-body sys-color-body"><!--<img src="/catalog/images/ts.gif" height="1" width="56">-->ציין את המיקום שבחרת להורדה,  ואז לחץ על <b>הורד כעת</b>. על מנת להסיר פריט מהסל שלך, לחץ על <b>הסר</b>.</div>
				</div>
				<div name="noreviewandinstall"id="noreviewandinstall" style="display:none">
					<div   class="8pt sys-font-body sys-color-body"><b><b>אין עדכונים בסל שלך</b></b></div>
					<div  class="8pt sys-font-body sys-color-body"><!--<img src="/catalog/images/ts.gif" height="1" width="56">-->כדי לבחור עדכונים להורדה לחץ על <b>ברוכים הבאים לקטלוג Windows Update </b> או קטגוריה מתחת לזה, והגש את קרטיריוני החיפוש שלך.</div>
				</div>
			</td>
		</tr>
		<tr>
			...
			<form name="txtform" id="textform" action="downloadbasket.asp" onsubmit="return downloadnow();">
				<div>
					<div class="8pt sys-font-body sys-color-body">&nbsp;<br><nobr>כדי להגיע למיקום עבור ההורדות שלך, הקלד מיקום או עיין:</nobr></div>
					<div class="10pt sys-font-heading4">
						...
						<button onresize="document.recalc('true')" style="width:expression(browsespan.offsetWidth + 10)" onClick="javascript:goBrowse()" id="browseb" name="browseb">
							<span id="browsespan" name="browsespan">עיון...</span></button></nobr>
					...
					<table cellpadding="0" cellspacing="0" border="0">
						<tr>
							<td class="8pt sys-font-body sys-color-body">
								...
								<button id="dnow" name="dnow" type="submit" style="width:expression(downspan.offsetWidth + 10)" disabled><span name="downspan" id="downspan">הורד עכשיו</span></button>
								<br>
							</td>
						</tr>
					</table>
		...
		<tr>
			<td class="resultsBasketBar sys-table-header-bgcolor1 sys-table-color-border sys-toppane-header-color" valign="top">
				<span id="eBasketCalculating">סך הכל (כולל דרושים מראש):</span> <span id="eBasketStats"></span>
			</td>
		</tr>
		<tr>
			<td height="100%">
...
//fnSortTable(2,false)
	...
	var Minute="דקה";
	var Minutes="דקות";
	var DownloadSize="גודל הקובץ להורדה:"
	var MB="MB"
	var KB="KB"
	var basketarray=parent.frames["eTOC"].sBasketString.split(",");
...
//alert(controlxml)
		...
		if(bstrStartFolder == "" || false == bstrStartFolder) {
			var badpath="ל- Windows Update אין אפשרות להוריד לתיק שציינת. ייתכן שהתיק אינו קיים או שאין לך הרשאות להוסיף לו פריטים. נא לודא שציינת נתיב נכון לתיק או ציין מיקום אחר.";
			alert(badpath);
			return false;
		}
	...
	function goBrowse() {
		...
		if(pbstrFolder.length >=50){
			...
			sAlert = 'נתיב ההורדה חייב לכלול פחות מ- 50 תווים. הנתיב שציינת הוא: \n\r' + pbstrFolder + "\n\r\n\r" + 'הקלד או לחץ על עיון כדי למצוא נתיב הורדה עם פחות מ- 50 תווים.'
			alert(sAlert)
		}
