	...
	<script language="JScript">
		...
		aRows = eSortTable.rows
			...
			for (i=0;i<buttonarray.length;i++){
				if(buttonarray[i].innerHTML == "Добавить")
					buttonarray[i].style.width = addlength
				if(buttonarray[i].innerHTML == "Удалить")
...
<table cellspacing="0" cellpadding="0" border="0">
	<tbody>
	<tr><td height="1"><span class="8pt sys-font-body sys-color-body" id="addspan" name="addspan" style="visibility:hidden;height=1;overflow;hidden">Добавить</span><span class="8pt sys-font-body sys-color-body" name="removespan" id="removespan" style="visibility:hidden;height=1;overflow;hidden">Удалить</span></td></tr>
		<tr>
			<td valign="top" height="40em">
				<div>
					<div valign="middle" style="background-color:#Ffffcc;padding-top:3;padding-bottom:5;padding-left:10">
						...
						<span class="title sys-font-heading3 sys-rhp-color-title">Корзина загрузки</span></nobr>
					</div>
				</div></td>
		</tr>
		<tr>
			<td valign="top">
				<div name="reviewandinstall"id="reviewandinstall">
					<div   class="8pt sys-font-body sys-color-body"><b>Просмотр и загрузка выбранных обновлений</b></div>
					<div  class="8pt sys-font-body sys-color-body"><!--<img src="/catalog/images/ts.gif" height="1" width="56">-->Выберите папку назначения и нажмите кнопку <b>Загрузить</b>. Чтобы удалить элемент из корзины, нажмите кнопку <b>Удалить</b>.</div>
				</div>
				<div name="noreviewandinstall"id="noreviewandinstall" style="display:none">
					<div   class="8pt sys-font-body sys-color-body"><b><b>В вашей корзине нет обновлений</b></b></div>
					<div  class="8pt sys-font-body sys-color-body"><!--<img src="/catalog/images/ts.gif" height="1" width="56">-->Чтобы выбрать обновления для загрузки, щелкните ссылку <b>Вас приветствует каталог Windows Update</b> или категорию в каталоге и задайте критерии поиска.</div>
				</div>
			</td>
		</tr>
		<tr>
			...
			<form name="txtform" id="textform" action="downloadbasket.asp" onsubmit="return downloadnow();">
				<div>
					<div class="8pt sys-font-body sys-color-body">&nbsp;<br><nobr>Введите путь или перейдите к папке, выбранной  для загрузки:</nobr></div>
					<div class="10pt sys-font-heading4">
						...
						<button onresize="document.recalc('true')" style="width:expression(browsespan.offsetWidth + 10)" onClick="javascript:goBrowse()" id="browseb" name="browseb">
							<span id="browsespan" name="browsespan">Обзор...</span></button></nobr>
					...
					<table cellpadding="0" cellspacing="0" border="0">
						<tr>
							<td class="8pt sys-font-body sys-color-body">
								...
								<button id="dnow" name="dnow" type="submit" style="width:expression(downspan.offsetWidth + 10)" disabled><span name="downspan" id="downspan">Загрузить</span></button>
								<br>
							</td>
						</tr>
					</table>
		...
		<tr>
			<td class="resultsBasketBar sys-table-header-bgcolor1 sys-table-color-border sys-toppane-header-color" valign="top">
				<span id="eBasketCalculating">Всего (включая предварительные требования):</span> <span id="eBasketStats"></span>
			</td>
		</tr>
		<tr>
			<td height="100%">
				<div id="eUpdatesContainer" class="eUpdatesContainer sys-table-color-border sys-table-cell-bgcolor2">
					...
					<div id="noupdates" class="8pt sys-font-body sys-color-body" style="display:none">
						...
						<p>
						<center>
						<b>В вашей корзине нет обновлений.</b>
						</center>
					</div>
...
//fnSortTable(2,false)
	...
	var Minute="мин.";
	var Minutes="мин.";
	var DownloadSize="Размер загрузки:"
	var MB="МБ"
	var KB="КБ"
	var basketarray=parent.frames["eTOC"].sBasketString.split(",");
...
//alert(controlxml)
		...
		if(bstrStartFolder == "" || false == bstrStartFolder) {
			var badpath="Программе Windows Update не удается выполнить загрузку в указанную папку. Либо папка не существует, либо у вас отсутствуют права записи в нее. Убедитесь, что путь к папке указан правильно или укажите другую папку.";
			alert(badpath);
			return false;
		}
	...
	function goBrowse() {
		...
		if(pbstrFolder.length >=50){
			...
			sAlert = 'Путь к папке назначения должен содержать не более 50 знаков. Указанный путь: \n\r' + pbstrFolder + "\n\r\n\r" + 'Введите или укажите, используя кнопку обзора, путь для загрузки, содержащий менее 50 знаков.'
			alert(sAlert)
		}
