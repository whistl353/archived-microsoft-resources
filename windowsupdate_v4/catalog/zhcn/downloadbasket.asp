	...
	<script language="JScript">
		...
		aRows = eSortTable.rows
			...
			for (i=0;i<buttonarray.length;i++){
				if(buttonarray[i].innerHTML == "添加")
					buttonarray[i].style.width = addlength
				if(buttonarray[i].innerHTML == "删除")
...
<table cellspacing="0" cellpadding="0" border="0">
	<tbody>
	<tr><td height="1"><span class="8pt sys-font-body sys-color-body" id="addspan" name="addspan" style="visibility:hidden;height=1;overflow;hidden">添加</span><span class="8pt sys-font-body sys-color-body" name="removespan" id="removespan" style="visibility:hidden;height=1;overflow;hidden">删除</span></td></tr>
		<tr>
			<td valign="top" height="40em">
				<div>
					<div valign="middle" style="background-color:#Ffffcc;padding-top:3;padding-bottom:5;padding-left:10">
						...
						<span class="title sys-font-heading3 sys-rhp-color-title">下载篮子</span></nobr>
					</div>
				</div></td>
		</tr>
		<tr>
			<td valign="top">
				<div name="reviewandinstall"id="reviewandinstall">
					<div   class="8pt sys-font-body sys-color-body"><b>复查并下载您选择的更新</b></div>
					<div  class="8pt sys-font-body sys-color-body"><!--<img src="/catalog/images/ts.gif" height="1" width="56">-->指定您选择的下载位置，然后单击<b>现在下载</b>。要从您的篮子中删除一个项目，单击<b>删除</b>。</div>
				</div>
				<div name="noreviewandinstall"id="noreviewandinstall" style="display:none">
					<div   class="8pt sys-font-body sys-color-body"><b><b>您的篮子中没有更新</b></b></div>
					<div  class="8pt sys-font-body sys-color-body"><!--<img src="/catalog/images/ts.gif" height="1" width="56">-->要选择要下载的更新，单击<b>欢迎使用 Windows Update 目录</b>或它下面的一个目录，然后提交您的搜索条件。</div>
				</div>
			</td>
		</tr>
		<tr>
			...
			<form name="txtform" id="textform" action="downloadbasket.asp" onsubmit="return downloadnow();">
				<div>
					<div class="8pt sys-font-body sys-color-body">&nbsp;<br><nobr>键入或浏览到您选择的下载位置:</nobr></div>
					<div class="10pt sys-font-heading4">
						...
						<button onresize="document.recalc('true')" style="width:expression(browsespan.offsetWidth + 10)" onClick="javascript:goBrowse()" id="browseb" name="browseb">
							<span id="browsespan" name="browsespan">浏览...</span></button></nobr>
					...
					<table cellpadding="0" cellspacing="0" border="0">
						<tr>
							<td class="8pt sys-font-body sys-color-body">
								...
								<button id="dnow" name="dnow" type="submit" style="width:expression(downspan.offsetWidth + 10)" disabled><span name="downspan" id="downspan">立即下载</span></button>
								<br>
							</td>
						</tr>
					</table>
		...
		<tr>
			<td class="resultsBasketBar sys-table-header-bgcolor1 sys-table-color-border sys-toppane-header-color" valign="top">
				<span id="eBasketCalculating">总共(包括先决条件):</span> <span id="eBasketStats"></span>
			</td>
		</tr>
		<tr>
			<td height="100%">
...
//alert(controlxml)
		...
		if(bstrStartFolder == "" || false == bstrStartFolder) {
			var badpath="Windows Update 不能下载指定的文件夹。文件夹不存在，或者您没有权限在其中添加项目。请确认您指定了到文件夹的正确路径，或者指定另一个位置。";
			alert(badpath);
			return false;
		}
	...
	function goBrowse() {
		...
		if(pbstrFolder.length >=50){
			...
			sAlert = '下载路径包含的字符必须少于 50 个。您指定的路径是: \n\r' + pbstrFolder + "\n\r\n\r" + '输入或浏览到下载路径，路径长度必须少于 50 个字符。'
			alert(sAlert)
		}
