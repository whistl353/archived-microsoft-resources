...
function fnWriteUpdates(sUp, bFirstcall) { 
	...
	if(bHidden){
		sHidden = "<table class='tblExclusiveSection' cellpadding=5 cellspacing=0><tr><td width=23  valign='top'><img src='shared/images/info_16x.gif'></td><td  valign='top'>" + parent.L_RListHiddenCriticalPresent1_Text + "<a href=\"javascript:parent.fnDisplayHiddenUpdates()\" style = \"text-decoration: underline\" >" +  parent.L_RListHiddenCriticalPresent2_Text + "</a>" + parent.L_RListHiddenCriticalPresent3_Text + "</td></tr></table>";
	...
	if((iExUpdateInBasket != -1) && !bExBasket) {
			...
			sExclusiveHeading = "<table class='tblExclusiveSection' cellpadding=5 cellspacing=0><tr><td width=23  valign='top'><img src='shared/images/info_16x.gif'></td><td  valign='top'>" + parent.L_RListSingleInstallation_Text + "</td></tr></table>";
			//resultsPageInfo.innerHTML = sExclusiveHeading;
			ePageDesc.innerHTML =sExclusiveHeading;
	} else {
		if(iSubPage == parent.conResultsBeta) {
			SelectallSpan.disabled = true;
			resultsPageInfo.innerHTML = "<table class='tblExclusiveSection' cellpadding=5 cellspacing=0><tr><td width=23  valign='top'><img src='shared/images/warning_icon.gif'></td><td  valign='top'>" + parent.L_RListBetaWarning1_Text + "" + parent.L_RListBetaWarning2_Text + "</td></tr></table>";
		}
	}
...
function fnWriteUpdate(iUpdate,  bLastInGroup,bSUperseded) {
	...
	sSizeText = parent.fnGetDownloadSizeText(iSize,parent.g_aUpdate[iUpdate].DownloadSec);
	if(iSize == 0) {
	...
	if(bExclusive) {
		...
		str = "<div id=\"Ttl" + iUpdate + "\" " + "class=\"gradNone " + sDivClass + "\">" + "\n" +
...
function fnSelectUpdates() {
	...
	if (!SelectallSpan.disabled) {
		...
		for(i = 0; i < iOptionalUpdates; i++) {
			...
			if(parent.g_aUpdate[iUpdate].MFDIndex !="" ){
				aMFDUpdateIndexes = parent.g_aUpdate[iUpdate].MFDIndex.split(",");
				for(ii=0; ii < aMFDUpdateIndexes.length-1; ii++) {
					iChildUpdateID = aMFDUpdateIndexes[ii];
					parent.g_UpdateCol(iChildUpdateID).IsHidden = false;
					parent.g_aUpdate[iChildUpdateID].IsHidden = false;
				}	
			} 
		}
	}
...
function fnSortArange(sUp) {
	...
	if(sUp != "") {
		...
		for(i = 0; i < aUpdates.length; i++) {
			j = aUpdates[i];
				if(((!parent.g_aUpdate[j].IsHidden && !parent.g_UpdateCol(j).IsMandatory ) || (iSubPage == parent.conResultsHidden)) &&  (parent.g_aUpdate[j].MFDIndex != "-1" )){
	...
	if(iOptionalUpdates >= 6 && (iSubPage != parent.conResultsBasket && iSubPage != parent.conExpressInstall && iSubPage != parent.conResultsHidden)) {
		SelectallSpan.innerHTML = "<br/><span onmouseup = 'fnSelectUpdates()'><input  onclick = 'return false' type = 'checkbox' id = 'SelectUpdates' name = 'SelectUpdates' ></span>" +  parent.L_RListSelectall_Text + "<br />";
	}
...
function fnModifyHidden(iUpdate) { 
	...
	var aMFDUpdateIndexes, ii, iChildUpdateID;
	...
	if(document.all["ckDecl" + iUpdate].checked) {
		...
		if(parent.g_aUpdate[iUpdate].MFDIndex !="" ){
			...
			for(ii=0; ii < aMFDUpdateIndexes.length-1; ii++) {
				iChildUpdateID = aMFDUpdateIndexes[ii];
				parent.g_UpdateCol(iChildUpdateID).IsHidden = true;
				parent.g_aUpdate[iChildUpdateID].IsHidden = true;
			}	
		} 
		document.all["Ttl" + iUpdate].className = "gradGrayBG";
	} else {
...
function fnWriteTotalSelected() {
	...
	for(i = 0; i < parent.g_aUpdate.length; i++) {
		if(parent.g_aUpdate[i].InBasket && !parent.g_UpdateCol(i).IsMandatory && (parent.g_aUpdate[i].MFDIndex != "-1")) {
			iSelected++;
			if(parent.g_aUpdate[i].IsDownloaded == false) {
				iSize += parent.g_aUpdate[i].Size;
				iSec += parent.g_aUpdate[i].DownloadSec
			}
		}
	}
	if (iSize == null) {
	...
	sSizeText = parent.fnGetDownloadSizeText(iSize, iSec);
	...
	if(iRadiovalue != -1) {
		...
		if(iSubPage==parent.conResultsHidden){
			for(j = 0; j < aExclusiveUpdates.length; j++) {
				...
				if(parent.g_aUpdate[aExclusiveUpdates[j]].MFDIndex !="" ){
					aMFDUpdateIndexes = parent.g_aUpdate[aExclusiveUpdates[j]].MFDIndex.split(",");
					for(ii=0; ii < aMFDUpdateIndexes.length-1; ii++) {
						iChildUpdateID = aMFDUpdateIndexes[ii];
						parent.g_UpdateCol(iChildUpdateID).IsHidden = true;
						parent.g_aUpdate[iChildUpdateID].IsHidden = true;
		...
		if(parent.g_aUpdate[iRadiovalue].MFDIndex !="" ){
			aMFDUpdateIndexes = parent.g_aUpdate[iRadiovalue].MFDIndex.split(",");
			for(ii=0; ii < aMFDUpdateIndexes.length-1; ii++) {
				iChildUpdateID = aMFDUpdateIndexes[ii];
				parent.g_UpdateCol(iChildUpdateID).IsHidden = false;
				parent.g_aUpdate[iChildUpdateID].IsHidden = false;
		...
		for(i = 0; i < parent.g_aUpdate.length; i++) {			
			if(iSubPage==parent.conResultsCritical && parent.g_aUpdate[i].IsCritical && !parent.g_aUpdate[i].IsHidden && (parent.g_aUpdate[i].MFDIndex!= "-1")){
	...
	switch(iSubPage) {
		case parent.conResultsCritical:
			...
			if(iExclusiveUpdates == 0 && iCriticalUpdates == 0){
				...
				PageDescription = "<table class='tblExclusiveSection' cellpadding=5 cellspacing=0><tr><td width=23  valign='top'><img src='shared/images/info_16x.gif'></td><td  valign='top'>" + parent.L_RList103_Text + "</td></tr></table>";
			}else if(iExclusiveUpdates > 1 && iCriticalUpdates == 0){
				PageDescription = parent.L_RList105a_Text + "<p><table class='tblExclusiveSection' cellpadding=5 cellspacing=0><tr><td width=23  valign='top'><img src='shared/images/info_16x.gif'></td><td  valign='top'>" + parent.L_RList105b_Text + "</td></tr></table>";
			}else if(iExclusiveUpdates > 0 && iCriticalUpdates > 0){
				PageDescription = parent.L_RList102a_Text +  "<p><table class='tblExclusiveSection' cellpadding=5 cellspacing=0><tr><td width=23  valign='top'><img src='shared/images/info_16x.gif'></td><td  valign='top'>" + parent.L_RList102b_Text + "</td></tr></table>";
			}else{
		...
		case parent.conResultsProduct:
			...
			if(iExclusiveUpdates == 0 && iOptionalUpdates == 0){
				...
				PageDescription = "<table class='tblExclusiveSection' cellpadding=5 cellspacing=0><tr><td width=23  valign='top'><img src='shared/images/info_16x.gif'></td><td  valign='top'>" + parent.L_RList203_Text + "</td></tr></table>";
			}else if(iExclusiveUpdates > 1 && iOptionalUpdates == 0){
				PageDescription = parent.L_RList205a_Text +  "<p><table class='tblExclusiveSection' cellpadding=5 cellspacing=0><tr><td width=23  valign='top'><img src='shared/images/info_16x.gif'></td><td  valign='top'>" + parent.L_RList205b_Text + "</td></tr></table>";
			}else if(iExclusiveUpdates > 0 && iOptionalUpdates > 0){
				PageDescription = parent.L_RList202a_Text +  "<p><table class='tblExclusiveSection' cellpadding=5 cellspacing=0><tr><td width=23 valign='top'><img src='shared/images/info_16x.gif'></td><td  valign='top'>" + parent.L_RList202b_Text + "</td></tr></table>";
			}else{
		...
		case parent.conResultsHidden:
			...
			if(iExclusiveUpdates == 0 && iOptionalUpdates == 0 && iCriticalUpdates == 0){
				...
				PageDescription= "<table class='tblExclusiveSection' cellpadding=5 cellspacing=0><tr><td width=23  valign='top'><img src='shared/images/info_16x.gif'></td><td  valign='top'>" + parent.L_RList405_Text + "</td></tr></table>";
			}else if(iExclusiveUpdates >1 ){
				PageDescription= "<table class='tblExclusiveSection' cellpadding=5 cellspacing=0><tr><td width=23  valign='top'><img src='shared/images/info_16x.gif'></td><td  valign='top'>" + parent.L_RList403_Text + "</td></tr></table>";
			}else if(iExclusiveUpdates == 1){
				PageDescription= "<table class='tblExclusiveSection' cellpadding=5 cellspacing=0><tr><td width=23  valign='top'><img src='shared/images/info_16x.gif'></td><td  valign='top'>" + parent.L_RList402_Text + "</td></tr></table>";
			}else{
...
function fnPingUninstalledUpdateInfo(){
	...
	if ('undefined' != typeof(parent.conWerMode) ){ //WER scenario
		if ( parent.conWerMode == parent.iWerQueryModeExpress ) {
			...
			for(i = 0; i < iUpdateCount; i++) { 
				if(parent.g_aUpdate[i].InBasket && parent.g_UpdateCol(i).Identity.UpdateID == parent.sWerUpdateId) {
					...
					fnPingServer(parent.g_aQueryString,"//go.microsoft.com/fwlink/?LinkId=23428&clcid=0x409"); 
				}
			}
		}
		else {
